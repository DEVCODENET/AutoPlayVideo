...
import static info.devcodenet.autoplayvideo.autoplayvideo.Utils.getString;
...
public class CustomRecyclerView extends RecyclerView {
    ...
    public void startDownloadInBackground(String url) {
        ...
        if ((getString(activity, url) == null || !(new File(getString(activity, url)).exists())) && url != null && !url.equalsIgnoreCase("null")) {
    ...
    public void preDownload(List<String> urls) {
        ...
        for (int i = 0; i < urls.size(); i++) {
            if ((getString(activity, urls.get(i)) == null || !(new File(getString(activity, urls.get(i))).exists())) && urls.get(i) != null && !urls.get(i).equalsIgnoreCase("null")) {
                Intent intent = new Intent(Intent.ACTION_SYNC, null, activity, DownloadService.class);
